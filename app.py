...
lottie_api = load_lottieurl('https://assets1.lottiefiles.com/packages/lf20_vsjw0irg.json')

# ---jupyter lottie transition---
# lottie_api = load_lottieurl('https://assets10.lottiefiles.com/private_files/lf30_8zz1umf1.json')
...
with st.form(key='form1'):
    ...
    car = st.selectbox('Car Model (Currently only these models are available)', options=carlist)
    car = dctcar[car]
    # location = st.text_input('Location')
    location = st.selectbox('Location (Currently only these cities are available)', options=loclist)
    ...
    power = st.text_input('Power in bhp (Enter upto six decimals for accuracy)')
    # power = int(power)
    price = st.text_input('Original Price in lakhs(Example: 13.56 lakhs)')


    # ---Submit Button---
    submit_button = st.form_submit_button(label='Check Price')
    # if submit_button:
